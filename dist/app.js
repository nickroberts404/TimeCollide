...
var time = [{ id: 0, start: 3.1, end: 5 }, { id: 1, start: 2, end: 8 }, { id: 2, start: 5, end: 6.2 }, { id: 3, start: 0, end: 9 }, { id: 4, start: 7, end: 9 }, { id: 5, start: 4, end: 6 }, { id: 7, start: 5.7, end: 6 }];
...
var intervals = (0, _interval2.default)(_data2.default);

(0, _setup2.default)('div.container');

var renderTime = function renderTime(time) {
	time = time.sort(function (d1, d2) {
		return d2.end - d2.start - (d1.end - d1.start);
	});
	var layer = d3.select('#time-layer');
	layer.selectAll('.time-block').data(time).enter().append('rect').attr('id', function (d) {
		return 'block-' + d.id;
	}).attr('class', 'time-block').attr('width', 50).attr('height', function (d) {
		return (d.end - d.start) * 20;
	}).attr('x', 0).attr('y', function (d) {
		return d.start * 20;
	}).style('fill', 'rgba(255, 0, 0, 0.1)').on('mouseover', function (d) {
		layer.select('#block-' + d.id).style('stroke', '#333');
	}).on('mouseleave', function (d) {
		layer.select('#block-' + d.id).style('stroke', '');
	});
};
var renderIntervals = function renderIntervals(interval) {
	var layer = d3.select('#time-layer');
	layer.selectAll('.time-interval').data(interval).enter().append('rect').attr('id', function (d) {
		return 'interval-' + d.id;
	}).attr('class', 'time-interval').attr('width', 50).attr('height', function (d) {
		return (d.end - d.start) * 20;
	}).attr('x', 60).attr('y', function (d) {
		return d.start * 20;
	}).style('fill', function (d) {
		return 'rgba(255, 0, 0,' + d.active.length / 10;
	}).on('mouseover', function (d) {
		layer.select('#interval-' + d.id).style('stroke', '#333');
		console.log('Active ids: ', d.active);
	}).on('mouseleave', function (d) {
		layer.select('#interval-' + d.id).style('stroke', '');
	});
};
renderIntervals(intervals);
renderTime(_data2.default);

},{"./data.js":3,"./interval.js":5,"./setup.js":6,"d3":1}],5:[function(require,module,exports){
'use strict';
...
*/
var patchIntervals = function patchIntervals(data) {
	var active = [],
	    intervals = [],
	    id = 0,
	    previous = undefined;
	data.forEach(function (d) {
		if (typeof previous !== 'undefined' && d.val != previous) {
			intervals = [].concat(_toConsumableArray(intervals), [{ start: previous, end: d.val, active: active.slice(), id: id++ }]);
		}
		if (d.type === 'start') active = [].concat(_toConsumableArray(active), [d.id]);else active = active.filter(function (a) {
			return d.id !== a;
		});
		previous = d.val;
	});
...
*/
var getIntervals = function getIntervals(data) {
	var patches = [];
	data.forEach(function (d) {
		return patches = [].concat(_toConsumableArray(patches), [{ type: 'start', val: d.start, id: d.id }, { type: 'end', val: d.end, id: d.id }]);
	});
	patches = patches.sort(function (d1, d2) {
		return d1.val - d2.val;
	});
	return patchIntervals(patches);
};

exports.patchIntervals = patchIntervals;
...
exports.appendLayer = exports.createSVG = undefined;
...
var appendLayer = function appendLayer(target, id, dim) {
	var layer = target.append('g').attr('id', id);
	if (dim) {
...
var createSVG = function createSVG(target) {
	var svg = d3.select(target).append('svg').attr('id', 'main-layer').attr('height', _constants.dimensions.height).attr('width', _constants.dimensions.width);

	appendLayer(svg, 'calendar-layer', _constants.dimensions);
	appendLayer(svg, 'time-layer', _constants.dimensions);
};
